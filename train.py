...
from tqdm import tqdm # progress line
...
def train(hyp, opt, device, tb_writer=None):
    ...
    #print("os.path.isfile(weights): ",os.path.isfile(weights))
    ...
    if rank in [-1, 0]:
        ...
        #print ("os.path.isfile(weights): ",os.path.isfile(weights))
        #print("weights.endswith('.pt'): ", weights.endswith('.pt'))
    ...
    #print("len(names): ",len(names)," nc:",nc)
    ...
    if pretrained:
        ...
        #print ("weights-for-ckpt",weights)
        ...
        #print("Model's state_dict:")
        kron = 0
        for param_tensor in ckpt['model'].state_dict():
            kron+=1
            if 0<=kron<=70:
    ...
    #print("End k-parametrs")
    #print ("nin: ",nin)
    # Optimizer
    nbs = 1  # nominal batch size
    #print ("total_batch_size: ",total_batch_size)
    accumulate = max(round(nbs / total_batch_size), 1)  # accumulate loss before optimizing
    #print ("accumulate: ",accumulate)
    hyp['weight_decay'] *= total_batch_size * accumulate / nbs  # scale weight_decay
    #print ("hyp-weight_decay", hyp['weight_decay'])
    ...
    if opt.adam:
        ...
        for num_mi, mi in enumerate(pg2):
            print("num_bias_size_{}: \n".format(num_mi),mi.size())
        for num_mi, mi in enumerate(pg0):
            print("num_bn_size_{}: \n".format(num_mi),mi.size())
        for num_mi, mi in enumerate(pg1):
            print("num_conv_size_{}: \n".format(num_mi),mi.size())
    ...
    #print("rect: ", opt.rect)
    #print("colorstr('train: ')", colorstr('train: '))
    ...
    # Model parameters
    #print("hyp['box']: ", hyp['box'])
    hyp['box'] *= 3. / nl  # scale to layers
    #print("hyp['box']: ",hyp['box'])

    #print("hyp['cls']: ",hyp['cls'])
    hyp['cls'] *= nc / 80. * 3. / nl  # scale to classes and layers
    #print("hyp['cls']: ",hyp['cls'])

    #print("hyp['obj']: ",hyp['obj'])
    hyp['obj'] *= (imgsz / 640) ** 2 * 3. / nl  # scale to image size and layers
    #print("hyp['obj']: ", hyp['obj'])

    #print("opt.label_smoothing: ",opt.label_smoothing)
    hyp['label_smoothing'] = opt.label_smoothing
    #print("hyp['label_smoothing']: ", hyp['label_smoothing'])
    ...
    #print("model.class_weights: ",labels_to_class_weights(dataset.labels, nc).to(device) * nc)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        #print ("opt.image_weights: ",opt.image_weights)
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # print ("plt.show")
            #plt.imshow(imgs[0][0].numpy(), cmap=plt.cm.gray)
            #plt.show()
            # Warmup
            #print("ni {} <= nw {}: ".format( ni,nw), ni <= nw)
            #print ("Warmup: ",ni <= nw)
            if ni <= nw:
                xi = [0, nw]  # x interp
            ...
            with amp.autocast(enabled=cuda):
                ...
                #print ("pred: ",pred )
                #print("pred.size_1: ", pred[0].size())
                #print("pred.size_2: ", pred[1].size())
                #print("pred.size_3: ", pred[2].size())
                #print("pred: ", pred)
            ...
            # Backward
            #print ("scaler.scale(loss): ",scaler.scale(loss))
            scaler.scale(loss).backward()

            # Optimize

            #print ("Optimize: ", ni ,"%", accumulate, "==", 0,",",ni % accumulate == 0)
            if ni % accumulate == 0:
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO 80
